...
You can run the dev mode on other port if you want. Just change the port in the webpack.dev.js config.


## Content Scripts
Although this boilerplate uses the webpack dev server, it's also prepared to write all your bundles files on the disk at every code change, so you can point, on your extension manifest, to your bundles that you want to use as [content scripts](https://developer.chrome.com/extensions/content_scripts). The content_scripts entry points need to be excluded from hot reloading [(why?)](https://github.com/samuelsimoes/chrome-extension-webpack-boilerplate/issues/4#issuecomment-261788690). To do so, simply include the entry points without including the 'webpack/hot/dev-server' in the `webpack.dev.js`. Look the example below.
...
To this task this boilerplate import the file `./secrets.<dev|prod>.js` on your modules through the module named as `secrets`, so you can do things like this:
